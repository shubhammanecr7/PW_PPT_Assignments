...
/*
💡 **Q5.** You are given two integer arrays nums1 and nums2, sorted in non-decreasing order, and 
...
public class Q5 {
	...
	private static void mergeSortedArrays(int[] nums1, int m, int[] nums2, int n) {
		...
		//using p pointer we place larger elements from last position
		while (p1 >= 0 && p2 >= 0) {
			// if curr element in nums1 >= curr element nums2. 
			//put nums1 element in result array
		...
		//now copying remaining elements from nums2 to nums1 if any
		while(p2 >=0 ) {
			nums1[p] = nums2[p2];
			p--;
			p2--;
